function [traj, traj_tau, dataReturn] = computeOptTraj(g, data, tau, dynSys, extraArgs)
...
traj_complete = nan(g.dim, tauLength*subSamples);
dVdX = nan(g.dim, tauLength*subSamples);
ctrls = nan(1, tauLength*subSamples);
dstbs = nan(1, tauLength*subSamples);
traj_complete(:,1) = dynSys.x;
...
while iter <= tauLength 
  ...
  for j = 1:subSamples
    ...
    traj_complete(:,(iter-1)*subSamples + j + 1) = dynSys.x;
    dVdX(:,(iter-1)*subSamples + j + 1) = deriv;
    ctrls(:, (iter-1)*subSamples + j + 1) = u;
    dstbs(:, (iter-1)*subSamples + j + 1) = d;
  end
...
dataReturn.state_traj = traj_complete';
dataReturn.dVdX = dVdX';
dataReturn.ctrls = ctrls(:, 2:end)';
dataReturn.dstbs = dstbs(:, 2:end)';
end